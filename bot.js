...
const { telegramToken, dbPath } = require('./settings');
...
bot.on('callback_query', async (query) => {
  ...
  switch (action) {
    ...
    case 'upload_list_txt':
      ...
      await bot.sendMessage(chatId, 'Please upload your list.txt file:');
      break;
  ...
  switch (userState.step) {
    ...
    case 'enter_email_body':
      ...
      await bot.sendMessage(chatId, 'Email body saved. You can now add recipients and attachments.', {
        reply_markup: {
          inline_keyboard: [
            [{ text: 'Upload list.txt', callback_data: 'upload_list_txt' }],
  ...
  try {
    ...
    fileStream.on('finish', async () => {
      if (userState.step === 'upload_letter_html') {
        ...
        userState.step = 'enter_rate_limit';
        await bot.sendMessage(chatId, 'list.txt uploaded successfully. Please enter the rate limit (messages per second):');
  ...
  try {
    for (const recipient of recipients) {
      const mailOptions = {
        ...
        html: bodyPath ? replacePlaceholders(fs.readFileSync(bodyPath, 'utf8'), recipient) : '',
